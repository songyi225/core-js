...
/* function first() {
  ...
  function second() {
    ...
    return x + y; // x를 찾을 수 있음
  }

  return second;
} */

//const result = first(); // result 내에 second 함수 그 자체가 담김
// 밖에 있는 변수인 result가 함수 내부에 접근해서 사용할 수 있는 형태를 클로저

//console.log(result()); // result가 함수가 되어서 실행할 수 있다
...
const result3 = first();
const result4 = first(); */
// 이런식으로 해주면 클로저 환경이 4개가 나옴.. 가비지 컬렉터가 수집해갈 수 없는 공간 4개가 생김
// 성능적으로 여러개 나오기때문에 메모리적으로 문제가 생기긴함 -> 근데 체감상 크게 문제는 안됨

console.log(result(30)); // second의 num으로 접근할 수 있다 -> 요걸 통해 x값이 바뀔 수 있음
console.log(first()(30)); // 위와 동일
