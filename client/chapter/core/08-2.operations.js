...
let lessThenAndEqual = number[3] <= number[1]; // 101 < 숫자로 바뀐 10
console.log('lessThenAndEqual: ', lessThenAndEqual); // false

let greaterThenAndEqual = number[0] >= number[1]; // 숫자 10 >= 문자 10
console.log('greaterThenAndEqual: ', greaterThenAndEqual); // true

// 동등(등급이나 정도가 같다) → boolean
let equal = number[0] == number[1]; // 숫자 10 == 숫자로 바뀐 10??
console.log('equal: ', equal); // true

// 다름(같지 않음) → boolean
let unequal = number[0] != number[1]; // 숫자 10 != 숫자로 바뀐 10?
console.log('unequal: ', unequal); // false

// 동일(똑같다) → boolean
let same = number[0] === number[1]; // 숫자 10과 문자 10은 같냐? 아님
